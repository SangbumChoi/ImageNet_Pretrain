...
class SplAtConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.radix > 1:
            splited = torch.split(x, rchannel // self.radix,
                                  dim=1)  # https://github.com/pytorch/pytorch/pull/32493/files
            gap = sum(splited)
        else:
            gap = x
...
class GDC(nn.Module):
    def __init__(self, in_c, embedding_size):
        ...
        self.conv_6_dw = Linear_block(in_c, in_c, groups=in_c, kernel=(8, 8), stride=(1, 1), padding=(0, 0))
